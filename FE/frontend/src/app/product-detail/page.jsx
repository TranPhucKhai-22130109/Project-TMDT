  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-300">
      <>
            ...
            <div className="fixed inset-y-0 left-0 z-[70] w-[80%] max-w-sm bg-white dark:bg-gray-900 shadow-2xl lg:hidden border-r border-gray-200 dark:border-gray-800 overflow-y-auto">
              ...
              <nav className="p-6 space-y-6">
                ...
                <div className="pt-6 border-t border-gray-200 dark:border-gray-800 space-y-4">
                  <Link contentKey="cta_12" className="block w-full py-3 px-4 text-center rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" href="login.html"> Log In </Link>
                  <Link contentKey="cta_13" className="block w-full py-3 px-4 text-center rounded-xl bg-gradient-to-r from-red-600 to-orange-500 text-white font-bold hover:shadow-lg hover:shadow-red-500/30 transition-all" href="login.html"> Sign Up Free </Link>
                </div>
              </nav>
            </div>
          )}
        ...
        {/* Nike Air Max 270 */}
        <section id="nike_air_max_270" className="py-12">
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              {/* Gallery */}
              <div className="space-y-4">
                <div className="relative aspect-square bg-gray-100 dark:bg-gray-700 rounded-2xl overflow-hidden group">
                  <div className="absolute top-6 left-6 bg-red-600 text-white px-4 py-2 rounded-full font-bold shadow-lg z-10"> -30% OFF </div>
                  <Image variant="cover" className="w-full h-full object-cover" alt="Product Image" />
                </div>
              ...
              <div className="space-y-8">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Text className="text-sm text-gray-500 dark:text-gray-400"> (428 reviews) </Text>
                  </div>
                  <h1 className="text-3xl lg:text-5xl font-black text-gray-900 dark:text-white uppercase italic tracking-tighter"> Nike Air Max 270 </h1>
                  <p className="mt-4 text-gray-600 dark:text-gray-300 leading-relaxed">
                     Legendary comfort, remixed. The Nike Air Max 270 features the first-ever Max Air unit created specifically for Nike Sportswear, delivering visible air under every step. 
                  </p>
                </div>
                {/* Price Block */}
                <div className="p-6 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg">
                  <div className="flex items-center gap-4 mb-4">
                    <Text className="text-5xl font-black text-red-600"> $139.30 </Text>
                    <div className="flex flex-col">
                      <Text variant="bold" className="text-gray-400 line-through font-bold text-lg"> $199.00 </Text>
                      <Text variant="bold" className="text-red-500 text-sm font-bold"> You save $59.70 (30%) </Text>
                    </div>
                  </div>
                  {/* Urgency */}
                  <div className="flex items-center gap-2 text-red-600 font-bold mb-6 animate-pulse">
                    <Flame className="w-5 h-5" />
                     Hurry! Only 3 pairs left in stock. 
                  </div>
                  {/* Actions */}
                  <div className="space-y-4">
                    <div className="flex gap-4">
                      <div className="w-32">
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1"> Size </label>
                        <select className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-3 outline-none focus:border-red-500">
                          <option> US 8 </option>
                          <option> US 9 </option>
                          <option> US 10 </option>
                          <option> US 11 </option>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900 w-32">
                          <Button contentKey="cta_14" className="px-3 py-3 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-l-lg"> - </Button>
                          <Input variant="text" className="w-full text-center bg-transparent outline-none" type="text" value="1" />
                          <Button contentKey="cta_15" className="px-3 py-3 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-r-lg"> + </Button>
                        </div>
                      </div>
                    </div>
                    <NextLink href="/cart" className="block w-full py-4 bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-700 hover:to-orange-600 text-white font-bold text-center rounded-xl shadow-lg shadow-red-500/30 transform hover:scale-[1.02] transition-all"> Add to Cart - $139.30 </NextLink>
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 pt-8">
                  <div className="flex gap-6 border-b border-gray-200 dark:border-gray-700 mb-6">
                    <button onClick={() => { setTab('desc') }} className={`pb-2 font-bold border-b-2 hover:text-red-600 transition-colors ${`${tab === 'desc' ? 'text-red-600 border-red-600' : ''} ${tab !== 'desc' ? 'text-gray-500 border-transparent' : ''}`}`}> Description </button>
                    <button onClick={() => { setTab('specs') }} className={`pb-2 font-bold border-b-2 hover:text-red-600 transition-colors ${`${tab === 'specs' ? 'text-red-600 border-red-600' : ''} ${tab !== 'specs' ? 'text-gray-500 border-transparent' : ''}`}`}> Specifications </button>
                    <button onClick={() => { setTab('shipping') }} className={`pb-2 font-bold border-b-2 hover:text-red-600 transition-colors ${`${tab === 'shipping' ? 'text-red-600 border-red-600' : ''} ${tab !== 'shipping' ? 'text-gray-500 border-transparent' : ''}`}`}> Shipping </button>
                  </div>
                  {tab === 'desc' && (
                    <div className="prose dark:prose-invert">
                      <p>
                         The Nike Air Max 270 React merges the boldest Max Air unit with Nike React foam for a cushioning sensation that moves smoothly from heel to toe. Aligned with the distinct visual identity of the Air Max 270. 
                      </p>
                    </div>
                  )}
                  {tab === 'specs' && (
                    <div style={{ display: "none" }}>
                      <ul className="space-y-2 list-disc pl-5 text-gray-600 dark:text-gray-400">
                        ...
                        <li> Color: Black/White-Dusty Cactus </li>
                        <li> Weight: 14oz </li>
                      </ul>
                    </div>
                  )}
                  {tab === 'shipping' && (
                    <div style={{ display: "none" }}>
                      <p className="text-gray-600 dark:text-gray-400">
                         Free standard shipping on orders over $100. Express delivery available. 
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                <form className="space-y-4">
                  <div className="relative">
                    ...
                    <Button contentKey="cta_17" className="absolute right-2 top-2 bottom-2 px-4 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold text-sm transition-colors" type="submit"> Subscribe </Button>
                  </div>
