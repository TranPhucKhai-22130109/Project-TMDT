...
export default function ProductDetailPanel({
  ...
  const stock = displayProduct.stockQuantity || 0;

  const stockPercentage = Math.min(100, (stock / 120) * 100);
  const stockColor =
    stockPercentage < 10
      ? "bg-red-500"
      : stockPercentage < 30
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          >
            <div className="mb-4">
              <img
                src={
                  displayProduct.images?.[activeImage] ||
              ...
              <div className="flex gap-2 mt-2">
                {(displayProduct.images || [displayProduct.imageUrl]).map(
                  (img, idx) => (
                    <button
                      key={idx}
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs font-mono rounded-md">
                {displayProduct.itemNo}
              </span>
              <span className="px-2 py-1 bg-indigo-50 text-indigo-700 text-xs font-medium rounded-md">
                {displayProduct.isAuction ? "Auction" : "Normal"}
              </span>
              <label className="relative inline-flex items-center cursor-pointer ml-auto">
                <input
                  type="checkbox"
                  checked={displayProduct.status}
                ...
                displayProduct.originalPrice > displayProduct.price && (
                  <span className="text-lg text-gray-400 line-through mb-1">
                    {new Intl.NumberFormat("vi-VN", {
                      style: "currency",
                      currency: "VND",
                    }).format(displayProduct.originalPrice)}
                  </span>
                )}
            </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h4 className="font-semibold text-gray-900 text-lg">
                      {displayProduct.sellerName || "Unknown Seller"}
                    </h4>
                  ...
                  <div className="mt-2 space-y-1 text-sm text-gray-600">
                    <div className="flex items-center gap-2">
                      ...
                      {displayProduct.sellerEmail || "No email"}
                    </div>

                    <div>
                      Seller ID:
                      <span className="font-mono ml-1 text-gray-800">
                        {displayProduct.sellerId}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-sm mb-6">
              {Object.entries({
                "Item No": displayProduct.itemNo,
                Scale: displayProduct.scale,
                Marque: displayProduct.marque,
                Stock: displayProduct.stockQuantity,
                Sold: displayProduct.soldQuantity,
                Type: displayProduct.isAuction ? "Auction" : "Normal",
              }).map(([key, value]) => (
                <div key={key}>
